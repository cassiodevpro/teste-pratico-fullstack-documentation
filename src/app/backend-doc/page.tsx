...
`}</code>
      ...
      <section>
        ...
        <ol className="list-decimal list-inside text-lg mb-4">
          <li>Abra o Postman e crie uma <b>Collection</b> chamada <span className="font-mono bg-[#233554] rounded px-1">Production Optimizer</span></li>
          <li>Crie um <b>Environment</b> <span className="font-mono bg-[#233554] rounded px-1">Local</span> com a variável <span className="font-mono bg-[#233554] rounded px-1">base_url = http://localhost:8080</span></li>
          <li>Use <span className="font-mono bg-[#233554] rounded px-1">{`{{base_url}}`}</span> em todas as requisições abaixo</li>
        </ol>
      </section>
      <hr className="border-[#233554]" />
      <section>
        <h3 className="text-xl font-bold mb-2">Matérias-Primas</h3>
        <div className="mb-2 text-lg">POST <span className="font-mono bg-[#233554] rounded px-1">{`{{base_url}}/api/raw-materials`}</span></div>
        <pre className="bg-[#233554] rounded p-4 text-xs overflow-x-auto mb-2">
          <code>{`{\n  "code": "MP001",\n  "name": "Farinha de Trigo",\n  "stockQuantity": 500.0\n}`}</code>
        </pre>
        <div className="mb-4 text-sm">Resposta esperada: <span className="font-mono bg-[#233554] rounded px-1">201 Created</span></div>
        <div className="mb-2 text-lg">GET <span className="font-mono bg-[#233554] rounded px-1">{`{{base_url}}/api/raw-materials`}</span></div>
        <div className="mb-4 text-sm">Resposta esperada: <span className="font-mono bg-[#233554] rounded px-1">200 OK</span> — array com todos os registros</div>
        <div className="mb-2 text-lg">PUT <span className="font-mono bg-[#233554] rounded px-1">{`{{base_url}}/api/raw-materials/1`}</span></div>
        <pre className="bg-[#233554] rounded p-4 text-xs overflow-x-auto mb-2">
          <code>{`{\n  "code": "MP001",\n  "name": "Farinha Integral",\n  "stockQuantity": 300.0\n}`}</code>
        </pre>
        <div className="mb-2 text-lg">DELETE <span className="font-mono bg-[#233554] rounded px-1">{`{{base_url}}/api/raw-materials/1`}</span></div>
        <div className="mb-4 text-sm">Resposta esperada: <span className="font-mono bg-[#233554] rounded px-1">204 No Content</span></div>
      </section>
      <hr className="border-[#233554]" />
      <section>
        <h3 className="text-xl font-bold mb-2">Produtos</h3>
        <div className="mb-2 text-lg">POST <span className="font-mono bg-[#233554] rounded px-1">{`{{base_url}}/api/products`}</span></div>
        <pre className="bg-[#233554] rounded p-4 text-xs overflow-x-auto mb-2">
          <code>{`{\n  "code": "PRD001",\n  "name": "Pao Frances",\n  "price": 0.75,\n  "ingredients": [\n    { "rawMaterialId": 1, "requiredQuantity": 50.0 },\n    { "rawMaterialId": 2, "requiredQuantity": 10.0 }\n  ]\n}`}</code>
        </pre>
        <div className="mb-2 text-lg">GET <span className="font-mono bg-[#233554] rounded px-1">{`{{base_url}}/api/products`}</span></div>
        <div className="mb-2 text-lg">DELETE <span className="font-mono bg-[#233554] rounded px-1">{`{{base_url}}/api/products/1`}</span> — <span className="font-mono bg-[#233554] rounded px-1">204 No Content</span></div>
      </section>
      <hr className="border-[#233554]" />
      <section>
        <h3 className="text-xl font-bold mb-2">Otimização de Produção</h3>
        <div className="mb-2 text-lg">GET <span className="font-mono bg-[#233554] rounded px-1">{`{{base_url}}/api/production/optimize`}</span></div>
        <pre className="bg-[#233554] rounded p-4 text-xs overflow-x-auto mb-2">
          <code>{`{\n  "suggestions": [\n    { "productName": "Bolo de Chocolate", "quantityToProduce": 3, "totalRevenue": 45.00 },\n    { "productName": "Pao Frances", "quantityToProduce": 10, "totalRevenue": 7.50 }\n  ],\n  "totalExpectedRevenue": 52.50\n}`}</code>
        </pre>
      </section>
      <hr className="border-[#233554]" />
      <section>
        <h3 className="text-xl font-bold mb-2">Cenários de erro</h3>
        <table className="w-full text-sm mb-4 border border-[#233554]">
          ...
          <tbody>
            ...
            <tr>
              <td className="p-2">Body inválido</td>
